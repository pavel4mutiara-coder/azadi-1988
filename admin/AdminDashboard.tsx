  ...
  return (
    <div className="space-y-14 animate-in fade-in duration-500 pb-20">
      ...
      <div className={`rounded-[3rem] p-1.5 border-2 transition-all duration-500 shadow-2xl ${cloudSynced ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-red-500/10 border-red-500/30 shadow-lg'}`}>
        <div className="bg-white dark:bg-slate-900 rounded-[2.8rem] p-8 flex flex-col items-center justify-between gap-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {[
          ...
          <div key={i} className="p-10 bg-white dark:bg-slate-900 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-xl group hover:border-emerald-500 transition-all flex flex-col items-center text-center gap-4">
        ...
        {ADMIN_NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className="group relative bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-lg hover:shadow-3xl hover:border-emerald-500 dark:hover:border-emerald-500 transition-all duration-500 flex flex-col items-center text-center gap-5 overflow-hidden"
            >
              ...
              <div className="w-14 h-14 rounded-[1.25rem] bg-slate-50 dark:bg-slate-950 text-slate-400 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 flex items-center justify-center group-hover:scale-110 transition-all shadow-inner border border-transparent group-hover:border-emerald-500/20">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[3.5rem] border border-slate-200 dark:border-slate-800 shadow-[0_30px_70px_-20px_rgba(0,0,0,0.2)] dark:shadow-[0_30px_70px_-20px_rgba(0,0,0,0.5)] overflow-hidden">
        <div className="p-10 border-b border-slate-100 dark:border-slate-800 flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex bg-slate-100 dark:bg-slate-950 p-2 rounded-2xl border border-slate-200 dark:border-slate-800/50">
          ...
          <div className="flex gap-4">
            <button onClick={() => setShowCashForm(!showCashForm)} className="bg-slate-900 dark:bg-white dark:text-slate-900 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-3 hover:opacity-90 shadow-2xl transition-all">
              <Plus size={18} /> {lang === 'bn' ? 'নগদ এন্ট্রি' : 'Add Cash'}
            </button>
            <button onClick={() => window.print()} className="p-4 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-300 rounded-2xl hover:bg-emerald-600 hover:text-white transition-all border border-slate-200 dark:border-slate-700 shadow-md">
              <Download size={22} />
            </button>
          </div>
        </div>

        {/* Form and Table sections keep their functional structure but benefit from global theme classes */}
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50 dark:bg-slate-950 text-[11px] font-black uppercase tracking-[0.25em] text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-slate-800">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
              {(activeTab === 'pending' ? pendingDonations : approvedDonations).map(d => (
                <tr key={d.id} className="hover:bg-slate-50 dark:hover:bg-slate-950/40 transition-all duration-300">
                  <td className="p-8">
                  ...
                  <td className="p-8">
                    ...
                    <div className="text-[10px] font-mono font-black text-slate-500 dark:text-emerald-400 bg-slate-100 dark:bg-emerald-950/40 px-3 py-1 rounded-lg inline-block uppercase tracking-wider border border-transparent dark:border-emerald-900/40">
                      {d.transactionId}
                    </div>
                  </td>
                  <td className="p-8">
                    <span className="text-[10px] font-black uppercase tracking-widest px-4 py-1.5 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded-full border border-emerald-100 dark:border-emerald-800">
                      {d.purpose}
                    </span>
                  </td>
                  ...
                  <td className="p-8 text-right space-x-3">
                    {d.status === DonationStatus.PENDING && (
                      <>
                        <button onClick={() => updateDonation(d.id, DonationStatus.APPROVED)} className="p-3 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-2xl hover:bg-emerald-600 hover:text-white transition-all shadow-sm">
                          <Check size={20} />
                        </button>
                        <button onClick={() => updateDonation(d.id, DonationStatus.REJECTED)} className="p-3 bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400 rounded-2xl hover:bg-rose-600 hover:text-white transition-all shadow-sm">
                          <X size={20} />
                        </button>
                      </>
                    )}
                    {d.status === DonationStatus.APPROVED && (
                      <button onClick={() => setViewingReceipt(d)} className="p-3 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-2xl hover:bg-slate-900 dark:hover:bg-white dark:hover:text-slate-900 hover:text-white transition-all shadow-md">
                        <FileText size={20} />
                      </button>
                    )}
