  ...
  return (
    <div className="space-y-16 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 bengali">
      ...
      <div className="text-center space-y-6 max-w-3xl mx-auto">
        <div className="inline-flex items-center gap-3 text-emerald-700 dark:text-emerald-300 font-black uppercase tracking-[0.2em] text-[10px] px-6 py-2.5 bg-emerald-100/50 dark:bg-emerald-900/40 rounded-full border border-emerald-200 dark:border-emerald-700 shadow-sm">
           <User size={14} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {sortedLeadership.map((leader) => (
          <div key={leader.id} className="bg-white dark:bg-slate-900 rounded-[3rem] border border-slate-200 dark:border-slate-800 p-8 shadow-xl group hover:-translate-y-2 hover:shadow-2xl transition-all duration-500 relative overflow-hidden flex flex-col items-center text-center">
            <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/5 dark:bg-emerald-400/5 rounded-bl-[4rem] group-hover:scale-110 transition-transform duration-700"></div>
            
            <div className="relative mb-6">
              <div className="w-32 h-32 rounded-full overflow-hidden border-4 border-emerald-500/10 group-hover:border-emerald-500 transition-all duration-500 bg-slate-50 dark:bg-slate-950 p-1 shadow-lg">
                {leader.image ? (
                  <img src={leader.image} alt={lang === 'bn' ? leader.nameBn : leader.nameEn} className="w-full h-full rounded-full object-cover transition-all duration-700 group-hover:scale-105" />
                ) : (
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="inline-block px-4 py-1.5 rounded-xl bg-emerald-50 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 font-bold text-[10px] uppercase tracking-widest border border-emerald-100 dark:border-emerald-800 bengali">
                {lang === 'bn' ? leader.designationBn : leader.designationEn}
              </div>
            </div>
            
            <div className="text-[13px] text-slate-600 dark:text-slate-300 italic bg-slate-50 dark:bg-slate-950/50 p-6 rounded-[2rem] w-full border border-slate-100 dark:border-slate-800 leading-relaxed font-bold flex-1 flex items-center justify-center bengali">
