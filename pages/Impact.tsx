...
export const Impact: React.FC = () => {
  ...
  const impactStats = [
    { label: lang === 'bn' ? 'উপকৃত পরিবার' : 'Beneficiary Families', val: '500+', icon: <Users />, color: 'text-blue-500 bg-blue-50 dark:bg-blue-900/20' },
    { label: lang === 'bn' ? 'সফল প্রজেক্ট' : 'Successful Projects', val: '24', icon: <Target />, color: 'text-emerald-500 bg-emerald-50 dark:bg-emerald-900/20' },
    { label: lang === 'bn' ? 'স্বেচ্ছাসেবক' : 'Volunteers', val: '120', icon: <Globe />, color: 'text-purple-500 bg-purple-50 dark:bg-purple-900/20' },
    { label: lang === 'bn' ? 'মোট সাহায্য' : 'Total Aid Distributed', val: '৳ ' + totalExpense.toLocaleString(), icon: <Heart />, color: 'text-pink-500 bg-pink-50 dark:bg-pink-900/20' },
  ];

  return (
    <div className="space-y-16 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="text-center space-y-4 max-w-2xl mx-auto">
        <h1 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white">{t.impact}</h1>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {impactStats.map((stat, i) => (
          <div key={i} className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-xl flex flex-col items-center text-center gap-4">
      ...
      <div className="bg-emerald-900 dark:bg-emerald-950 text-white rounded-[3rem] p-10 md:p-16 shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-500/20 blur-3xl rounded-full -mr-32 -mt-32"></div>
        <div className="relative z-10 grid md:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <h2 className="text-3xl font-black">{lang === 'bn' ? 'আমাদের অর্থায়নের উৎস' : 'Our Funding Sources'}</h2>
          ...
          <div className="flex justify-center">
            <div className="w-64 h-64 bg-white/10 rounded-full flex items-center justify-center border-8 border-white/5 relative">
              <PieChart size={120} className="text-white opacity-20" />
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4">
                 <div className="text-sm font-bold opacity-60 uppercase tracking-widest">Income</div>
